...
const DAY_NAME = format(new Date(), 'EEEE')
...
export default function TruckInventoryPage() {
  ...
  const loadCurrent = useCallback(async () => {
    ...
    // Initialize counts from current on hand
  ...
  return (
    <div className="p-8">
      <PageHeader
        ...
        sub={`${DAY_NAME} · ${format(new Date(), 'MMM d, yyyy')}`}
      />

      <div className="flex items-center justify-between mb-6">
        <LocationSelector onChange={setLocationId} filter="food_truck" />
        <div className="flex gap-2">
          {tabBtn('current', <Truck className="w-4 h-4" />, 'Current & Count')}
              ...
              <div className="bg-brand-50 border border-brand-100 rounded-xl p-4 flex items-center justify-between">
                <div>
                  <p className="text-sm font-semibold text-brand-800">
                    Current Inventory Snapshot — {format(new Date(), 'EEEE, MMM d')}
                  </p>
