...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      router.push(data.role==='lc_truck'||data.role==='salem_truck' ? '/truck-inventory' : '/dashboard')
      router.refresh()
        ...
        <form onSubmit={handleSubmit} className="bg-stone-800 rounded-2xl p-6 shadow-xl space-y-4">
          <div>
            ...
            <input type="text" value={username} onChange={e=>setUsername(e.target.value)}
              ...
              className="w-full px-4 py-3 bg-stone-700 text-white rounded-xl border border-stone-600 focus:outline-none focus:ring-2 focus:ring-amber-600 placeholder-stone-500 text-base"
              autoFocus />
          </div>
          <div>
          ...
          {error && <p className="text-red-400 text-sm text-center">Invalid username or password</p>}
          <button type="submit" disabled={loading||!username||!password}
            className="w-full py-3.5 bg-amber-700 hover:bg-amber-600 text-white font-semibold rounded-xl transition-colors disabled:opacity-50 text-base">
