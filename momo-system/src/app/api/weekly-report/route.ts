...
export async function GET(req: NextRequest) {
    ...
    for (const loc of targetLocations) {
      ...
      try {
        ...
        for (const order of allOrders) {
          ...
          // Refunds tracked for reporting only, NOT subtracted from netRevenue
  ...
  // order.net_amounts.total_money already excludes returns + discounts;
  // strip tips/tax/service charges to get business income. Do NOT subtract refunds again.
  const netRevenue = grossOrderTotal - tipTotal - taxTotal - serviceChargeTotal
  const grossRevenue = netRevenue // before display refunds
